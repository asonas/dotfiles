...
- entity ページや concept ページは、同じ対象について時系列の事実を統合してよい。ただし、独立した概念・実験・設計判断・再利用可能な手順として単独で読める内容は、別のソースノートに切り出し、wiki ページには結論と位置づけだけを統合する。
- 実験記録と設計判断は `notes/` に一次ソースとして保存する。ファイル名は `<topic>-experiment.md` または `<topic>-decision.md` のように、対象と記録の種類が分かるものにする。wiki は実験ログや判断の経緯を正典として持たない。
- ingest で既存ページに独立した内容が混在していると判断した場合、既存本文を自動分割せず `split-candidate:` として log に報告する。分割は内容の出典とリンクを確認してから行う。

## ノート作成後の深掘り提案

新しい wiki ページまたは承認済みの `notes/` ソースノートを書き終えたら、本文をもう一度確認する。本文中で説明のために触れただけの関連用語・隣接概念など、まだ独立ノートになっていないが深掘りする余地がある候補（以下、深掘り候補）があれば、既存ページと重複しないことを確認し、「〇〇についても別ノートとして切り出して調べられそうですが、書きますか？」のように提案する。

既存ページを分割した直後も、分割後の各本文を同じ観点で確認する。候補がなければ無理に提案しない。候補の提示は提案に留め、深掘り候補のノート作成・分割・MOCは自動作成しない。ユーザーの承認後にだけ切り出し、作成後は出典・リンク・粒度を検証する。

## 手を動かす実験

説明を読むだけでは理解が浅くなりやすい技術トピックでは、可能で安全な範囲でコードを書いて動かす手を動かす実験を優先的に検討する。特にネットワークやカーネル周りは、最小限の実験で挙動を確かめてから記録する。実験が不要、実行環境がない、またはリスクを説明できない場合は、無理に実行せずその理由を残す。

実験コードを実行する前に、実行するコマンド、触るデバイスやリソース、想定される影響を説明し、必ずユーザーの許可を得る。許可を得る前にコードのビルド・実行、デバイスへのアクセス、外部状態を変更する操作を行わない。

実験を行った場合は、対象概念のノートに手順や経緯を混ぜ込まず、`<topic>-experiment.md` の独立した実験ノートに切り出す。実験ノートには、目的、使った素材（参照したリポジトリ等）、実際に躓いた点とその対処、再現に必要なコマンドと実行結果、コードから読み取れることを書く。単なる感想ではなく再現可能な記録にし、概念ノート側からリンクする。1ノート1概念の原則を維持し、概念ノートには実験の結論と位置づけだけを統合する。

## MOC候補

- 同じトピックに属する wiki ページが 3つ以上たまり、相互リンクだけでは各ページの位置づけが分かりにくくなった場合は `moc-candidate:` として報告する。候補の検出と報告は `lint` で行い、`ingest` 中に MOC を作成しない。
- MOC の候補生成は自動化してよいが、MOCページの作成はユーザの承認後に行う。作成する場合は `wiki/` のフラット構造を維持し、`type: summary` のページとして、配下ページへの wikilink と各ページの役割を短く記述する。
- `wiki/index.md` は type 別の自動カタログであり、MOC の代わりにはしない。MOCに事実や判断を記述する場合は、通常の wiki ページと同じく `sources:` と本文内の出典リンクを付ける。

## 外部情報と自分の考察

- bookmark や Web 調査などの外部情報を統合するページでは、出典から確認できる内容と `自分の考察` を同じ段落に混ぜない。必要に応じて `出典の整理`、`自分の考察`、`実験結果` の見出しを使い分ける。考察に含めてよいのは解釈や判断であり、出典にない事実を事実として追加してはならない。仮説を残す場合は `推測` または `仮説` と明示し、検証可能な出典がある主張はその出典を追加する。
- この分離は外部情報と実験記録に適用し、通常の daily 由来の作業履歴には一律の見出しを追加しない。frontmatter の `sources:` と本文内の wikilink による出典規約は、どのページでも維持する。
...
7. **更新 or 新規作成**:
   ...
   - 外部情報や実験結果を含む場合は、出典から確認できる内容と自分の考察を見出し単位で分離する。wikiページに実験手順や設計判断の経緯を丸ごと転記しない。
...
9. **ソースノート側へのリンク追加**: 現在のソース note 本文に該当語が出現していて wikilink になっていない場合、CLAUDE.md のリンク戦略に従って初出のみ `[[語]]` を付ける。新規 wiki ページを作成したときは、`daily/`、`activities/`、`notes/`、`essays/`、`projects/`、`1on1/`、`bookmarks/`、`books/` にある関連する過去のソース note を確認し、ページ名と完全一致する語の本文（frontmatter とコードブロックを除く）での初出だけに wikilink を追加してよい。初出がすでに wikilink なら、後続の出現箇所へ移って追加してはならない。`wiki/`、`index.md`、`log*.md`、`deferred.md` は対象外とする。過去のソース note の本文は書き換えず、候補語の言い換え・段落整理・一括リンク追加は行わない。追加したノートは `links:` として log に記録する。
10. **log.md への記録**: `wiki/log.md` の先頭セクション直下に新エントリを追加（append-only。古いエントリは削除しない）:
   ...
   - links: [[A]] 作成時に過去のソース note 3件へ wikilink を追加
...
2. 各ページについて以下をチェック:
   ...
   - **分割候補**: entity / concept ページに、独立した概念・実験・設計判断・再利用可能な手順が混在している。自動分割せず `split-candidate:` に報告する
   - **MOC候補**: 同一トピックに属するページが 3つ以上あり、共通するリンクや出典から概念的なまとまりを作れる。`lint` で検出し、自動作成せず `moc-candidate:` に報告する
3. **自動修復（その場で直すもの）**:
...
5. **報告のみ（判断が必要なもの）**: 孤立ページ・矛盾・統合候補・分割候補・MOC候補・出典が見つからないページ。分割候補とMOC候補はこの段階で作成せず、検出結果を `wiki/log.md` の先頭に新エントリとして追加:
   ...
   - split-candidate: [[A]] の実験・設計判断を [[B]] として分離可能（判断待ち）
   - moc-candidate: [[MOC候補]]（[[A]], [[B]], [[C]]。作成は承認後）
   - links: [[A]] 作成時に過去のソース note 3件へ wikilink を追加
...
- `wiki/LLM Wiki.md` は、LLM Wikiのページ構造・出典・分割・MOC・リンク追加に関する正典の運用規約です。このスキルの手順を更新するときは、先にそのページとの整合を確認すること。
