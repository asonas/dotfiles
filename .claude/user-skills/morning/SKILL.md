---
name: morning
description: Use when starting the workday. Conducts a scrum-style daily standup (Inspect prior day, Adapt today's plan against quarterly goals), gathering calendar / Gmail / Linear / Things3 / Obsidian context and creating today's Obsidian daily note.
...
取得した events 配列を JSON 配列として `/tmp/activities-calendar-events.json` に書き出す (Step 6a で activities リポジトリにインポートするため)。書き出しは Write ツールで以下のような JSON 配列形式にする:
...
#### 6a: カレンダー予定 → activities ファイル

Step 2 で取得した GCal events を **activities リポジトリ** にインポートし、`activities/YYYY-MM-DD.md` のカレンダーセクションに反映する。Things3 への個別タスク登録は廃止 (Phase 5 で撤去)。

Step 2 で取得した events 配列を `/tmp/activities-calendar-events.json` のような一時ファイルに JSON 配列として書き出し、以下を実行する:
...
「## 今日の予定」は activities ファイル (Step 6a で更新済み) からの transclude にする。手書きで table を組まないこと。
