...
以下 3 つの情報収集タスクを **一度のメッセージで並列に Agent を起動** し、Sonnet モデルに処理させる。各 subagent は調査結果を構造化された短いテキスト（200〜400 字目安）でメインスレッドに返す。すべての subagent は `model: "sonnet"`、`subagent_type: "general-purpose"` で起動する。

**並列起動の鉄則:** 3 つの Agent tool 呼び出しを **同一のメッセージ内** に配置する。1 つずつ順に呼ぶと並列にならない。
...
#### 4b. gmail-digest subagent

Linear 通知メール / Datadog Daily Digest を要約する subagent。

プロンプト要旨:
```
あなたは asonas の Gmail から Linear / Datadog のデジェスト情報を抽出する任務を持つ。
~/.claude/skills/gmail-digest/SKILL.md を Read で読み、その指示の Step 1〜3 までを実行してほしい。
**daily note への append は実行しないこと。** メインスレッド側で行う。
代わりに、以下フォーマットで構造化された要約をテキストで返す:

## Linear通知まとめ（本文ドラフト）
<散文。CLAUDE.mdの文章スタイルに従う。issue IDは [[CTI-XXX]] 形式の wikilink>

## Datadog Daily Digest (MM/DD分)
<散文。サービス名は [[Datadog]] 等の wikilink>

該当メールがなければ "該当なし" と書く。
```

#### 4c. linear-tracker subagent
...
```
（同一メッセージ内に以下 3 つの Agent tool 呼び出しを並列で配置する）
...
3 つの結果を全てメモリに保持し、後段の Step 6 / Step 7 で利用する。
...
### Step 6.5: Append Gmail Digest Sections

Step 4b (gmail-digest subagent) の返却本文を、daily note の末尾に追記する。

```bash
obsidian append vault=asonas path="daily/YYYY-MM-DD.md" content="<Step 4b の Linear通知まとめドラフト>\n\n<Step 4b の Datadog Daily Digest 本文>" 2>/dev/null
```

「該当なし」のセクションは append しない。
...
- **自分のアクティブLinear Issue**: Step 4c の結果から上位を提示
- **チームの停滞Linear Issue**: Step 4c の結果から件数と特に古いものを提示（フォローアップが必要なら今日やることに入れる候補として）
