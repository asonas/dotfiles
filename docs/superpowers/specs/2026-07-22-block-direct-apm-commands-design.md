...
raw Bash commandから単一引用符と二重引用符を除去し、backslashと改行の組を連結する軽い正規化だけを行う。
正規化後の文字列中に、パス付きも含む `apm update` または `apm install` の対象形があれば、次の形式で拒否を返す。
...
判定対象には、オプション付きのコマンド、絶対パスまたは相対パスで指定された `apm`、複合コマンド、コマンド置換、backtick、引用符断片、前置代入を含める。
厳密な実行位置やシェル構文は解析しない。そのため、コメント、引用文字列、引数、代入値に対象形がある場合も保守的に拒否する。
...
その後、引用符断片、コメント、引数、代入値、コマンド置換に対象形があれば拒否されることを確認する。
`apm compile`、`apm --version`、`./install.sh`、空入力、commandなしは許可されることを確認する。
...
- [x] 対象形を含む文字列を実行位置にかかわらず拒否し、他のAPMサブコマンドを許可する。
- [x] `capm` や `myapm` など別識別子の末尾を対象APMとして扱わない。
- [x] `./install.sh` の実行を許可する。
- [x] 全シェルテストと構文検査を実行する。
- [x] Bash用PreToolUse設定からHookを呼び出す。
...
- 2026-07-22：正式レビューに基づき、二重引用符内のバッククォート置換、引用付きサブコマンド、前置代入、代入値と実行パスの分離を追加した。
- 2026-07-22：再レビューに基づき、引用前後のトークン連結と代入トークンの字句的な読み飛ばしを追加した。
- 2026-07-22：3回のレビュー修正で手製パーサの限界が判明し、ユーザー判断で保守的検出へ変更した。
- 2026-07-22：保守的検出への変更後に対象テスト、全シェルテスト、構文検査、settingsのJSON検査、差分検査が成功した。
- 2026-07-23：Bash用PreToolUseにHookを登録し、Hook登録テスト、全シェルテスト、構文検査、settingsのJSON検査、差分検査がすべて終了コード0で成功した。
- 2026-07-23：`apm` の前方境界を追加し、`capm update` と `myapm install` を許可した。
