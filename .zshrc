...
# 非ログインの zsh サブシェルでは再発火させない。第2引数は付けず macism のデフォルト workaround
# (150ms) を効かせる。macOS 26 (Tahoe) では入力ソース選択がアクティベーション周期を経るまで確定
# しないため、workaround を切ると (第2引数 0) pane 生成時の切替が確定せず日本語のまま残ることがある。
